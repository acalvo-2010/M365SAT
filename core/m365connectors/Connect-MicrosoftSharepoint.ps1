    ...
    while ($attempt -le $maxAttempts) {
        try {
            ...
            if ($null -ne ((Get-MgApplication | Where-Object { $_.DisplayName -eq 'PnP Rocks' }).AppId)){
                ...
                $ClientId = Register-PnPEntraIDApp -ApplicationName "PnP Rocks" -Tenant "$TenantName.onmicrosoft.com" -Interactive -AzureEnvironment $SpEnvironment -ErrorAction SilentlyContinue
